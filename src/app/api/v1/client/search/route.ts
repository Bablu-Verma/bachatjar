...
export async function POST(req: Request) {
  ...
  try {
    ...
    const [ stores, coupons, campaigns, referral] = await Promise.all([
      ...
      CampaignModel.find({ title: searchFilter, product_status: "ACTIVE" }).limit(6).populate('store', 'name cashback_type store_type cashback_rate store_link store_img').populate('category', 'name slug').select('store extrnal_url category offer_price calculated_cashback calculation_mode product_img product_tags  actual_price product_slug slug_type title  createdAt updatedAt _id').lean(),
