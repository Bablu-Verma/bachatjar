...
const Deals: React.FC<DealsProps> = ({ best_product }) => {
  const [openTab, setOpenTab] = useState<"hot_deals" | "live_offer">(
    "hot_deals"
  );
  ...
  const handleTabChange = (tab: "hot_deals" | "live_offer") => {
    setOpenTab(tab);
    setPage(1);
    setHasMore(true);
  };
  return (
    <div className="max-w-6xl mx-auto pt-2 mb-4 relative">
      <div className="flex px-2 justify-start items-center py-4 gap-6">
        <button
          onClick={() => handleTabChange("hot_deals")}
        ...
        <button
          onClick={() => handleTabChange("live_offer")}
          className={`text-sm py-1 px-6 transition-all duration-300 ease-in-out rounded-full border-2 border-primary ${
            openTab === "live_offer"
              ? "text-white bg-primary"
              : "text-primary bg-white"
...
const LiveDealCard = ({ item }: any) => {
  ...
  return (
    ...
    >
      <span className="absolute top-5 left-0 bg-primary text-white z-30 text-[12px]  py-.5 px-3 rounded-r-[2px]">
        Live
      </span>
