...
export async function amazonElectronsScrape(): Promise<Product[]> {
  ...
  await page.setUserAgent(getRandomDesktopUserAgent());
    ...
    if (item.store_name.toLowerCase() === 'amazon') {
      ...
      if (isDirectAmazonUrl) {
        ...
        try {
          ...
          await redirectPage.setUserAgent(getRandomDesktopUserAgent());
...
export async function amazonDealScrape(): Promise<Product[]> {
  const browser = await puppeteer.launch({ headless: true });
  const page = await browser.newPage();
  await page.setUserAgent(getRandomDesktopUserAgent());
  ...
  await page.waitForSelector('.l-deal-box');

  const rawProducts = await page.$$eval('.l-deal-box', cards => {
    return cards.slice(0, 10).map(card => {
      const title = card.querySelector('.l-deal-dsp a')?.textContent?.trim() || '';
      const store_name = card.querySelector('.l-deal-store a')?.textContent?.trim() || '';
      const priceText = card.querySelector('.l-deal-price')?.textContent?.replace(/[^\d]/g, '') || '';
      const price = parseFloat(priceText) || 0;
      const discountText = card.querySelector('.l-deal-discount')?.textContent?.trim() || '';
      const discount = parseFloat(discountText.replace(/[^\d]/g, '')) || 0;
      const image = card.querySelector('.l-deal-box-image img')?.getAttribute('src') || '';
      const url = card.querySelector('.btn-lgetdeal.home-page-get-deal')?.getAttribute('data-href') || '';
  
      // Calculate actual price (MRP) if discount available
      let actual_price = 0;
      if (price > 0 && discount > 0) {
        actual_price = Math.round(price / (1 - discount / 100));
      }
  
      return { title, price, actual_price, discount, image, store_name, url };
    });
  });
        ...
        try {
          ...
          if (asin) {
            ...
            finalResults.push({
              ...
              image: item.image,
              actual_price: item.actual_price.toString() || null,
              category: categoryId || '684d935e0ea1a8196dda777a',
              user_id: '6842c24a2b033a25c4bc0bc7',
              store: '6892e660f2d3213a3c4cb5ce',
              url: `https://www.amazon.in/dp/${asin}/?tag=${aff_id}`,
